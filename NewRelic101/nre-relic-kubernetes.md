...
The same goes for a node level. If the node reaches a high CPU or memory state, there is really nothing a team member can do. Either the node will go back to a normal state, or a pod that is taking too many resources will get evicted and rescheduled, and the cluster autoscaler should automatically prepare a new machine for the evicted pod. If a new node fails to come up, or the evicted pod takes too long to reschedule, then alerting makes sense.