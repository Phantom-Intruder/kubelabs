...
One important rule of alerting is that you should only alert to necessary things. If you keep firing off alerts all the time, the team member who receives the alerts will start being ignored or de-prioritized. This is especially true if there is nothing that the person can do about the alert. For example, if the CPU usage is high and there is a scaler in place to scale up based on CPU usage, unless the CPU scaler is not working and the pods are getting throttled, alerting should not be happening. If the alert goes off and all the user has to do is sit and watch the new pods scale up, then there is no point in the user being there at all. If the new pod doesn't scale up or if it starts crashing, or is unable to get into a ready state, that is the point at which the user should get involved.

The same goes for a node level. If the node reaches a high CPU or memory state, there is really nothing a team member can do. Either the node will return to a normal state, or a pod that is consuming too many resources will be evicted and rescheduled, and the cluster autoscaler should automatically prepare a new machine for the evicted pod. If a new node fails to come up, or the evicted pod takes too long to reschedule, then alerting makes sense. Ultimately, consider your cluster setup to minimize noise when alerting. This way, whenever an alert is triggered that signals an urgent issue that needs to be checked immediately, it will be addressed. 